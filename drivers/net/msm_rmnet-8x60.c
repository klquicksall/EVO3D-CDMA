...
static int msm_rmnet_debug_mask=5;
...
static __be16 rmnet_ip_type_trans(struct sk_buff *skb, struct net_device *dev)
{
	...
	case 0x60:
		...
		break;
	default:
		pr_err("[RIL][%s] rmnet_recv() L3 protocol decode error: 0x%02x",
...
static void smd_net_data_handler(unsigned long arg)
{
	...
	for (;;) {
		...
		if (skb == NULL) {
			pr_err("[RIL][%s] rmnet_recv() cannot allocate skb\n",
				       dev->name);
			} else {
				skb->dev = dev;
				...
				if (smd_read(p->ch, ptr, sz) != sz) {
					pr_err("[RIL][%s] rmnet_recv() smd lied about avail?!",
						dev->name);
					ptr = 0;
...
static int _rmnet_xmit(struct sk_buff *skb, struct net_device *dev)
{
	...
	if (smd_ret != skb->len) {
		pr_err("[RIL][%s] %s: smd_write returned error %d",
...
static void *msm_rmnet_load_modem(struct net_device *dev)
{
	...
	pil = pil_get("modem");
	if (IS_ERR(pil))
		pr_err("[RIL][%s] %s: modem load failed\n",
			dev->name, __func__);
		...
		if (!rc)
			rc = -ETIMEDOUT;
		if (rc < 0) {
			pr_err("[RIL][%s] %s: wait for rmnet port failed %d\n",
...
static int rmnet_change_mtu(struct net_device *dev, int new_mtu)
{
	...
	DBG0("[RIL][%s] MTU change: old=%d new=%d\n",
...
static int rmnet_xmit(struct sk_buff *skb, struct net_device *dev)
{
	...
	if (netif_queue_stopped(dev)) {
		pr_err("[RIL][%s] fatal: rmnet_xmit called when netif_queue is stopped",
			dev->name);
		return 0;
	}
...
{
	pr_warning("[RIL][%s] rmnet_tx_timeout()\n", dev->name);
}
...
static int rmnet_ioctl(struct net_device *dev, struct ifreq *ifr, int cmd)
{
	...
	case RMNET_IOCTL_SET_LLP_IP:        /* Set RAWIP protocol      */
		...
		if (p->operation_mode & RMNET_MODE_LLP_ETH) {
			...
			p->operation_mode |= RMNET_MODE_LLP_IP;
			spin_unlock_irqrestore(&p->lock, flags);
			DBG0("[RIL][%s] rmnet_ioctl(): set IP protocol mode\n",
				dev->name);
		}
		break;
	...
	case RMNET_IOCTL_SET_QOS_ENABLE:    /* Set QoS header enabled  */
		...
		spin_unlock_irqrestore(&p->lock, flags);
		DBG0("[RIL][%s] rmnet_ioctl(): set QMI QOS header enable\n",
			dev->name);
		break;
	...
	case RMNET_IOCTL_SET_QOS_DISABLE:   /* Set QoS header disabled */
		...
		p->operation_mode &= ~RMNET_MODE_QOS;
		spin_unlock_irqrestore(&p->lock, flags);
		DBG0("[RIL][%s] rmnet_ioctl(): set QMI QOS header disable\n",
			dev->name);
		break;
	...
	case RMNET_IOCTL_OPEN:              /* Open transport port     */
		rc = __rmnet_open(dev);
		DBG0("[RIL][%s] rmnet_ioctl(): open transport port\n",
			dev->name);
		break;

	case RMNET_IOCTL_CLOSE:             /* Close transport port    */
		rc = __rmnet_close(dev);
		DBG0("[RIL][%s] rmnet_ioctl(): close transport port\n",
			dev->name);
		break;

	default:
		pr_err("[RIL][%s] error: rmnet_ioct called for unsupported cmd[%d]",
			dev->name, cmd);
		return -EINVAL;
	}

	DBG2("[RIL][%s] %s: cmd=0x%x opmode old=0x%08x new=0x%08x\n",
